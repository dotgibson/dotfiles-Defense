...
| `note <text>` | Append a timestamped line to the active case's `notes/notes.md` (`$CASE/notes`, falls back to `$PWD/notes`); no-arg is a usage error, so the audit trail never gets an empty entry |
