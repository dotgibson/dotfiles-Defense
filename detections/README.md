...
**Multi-record events.** An auditd event is several records — the `SYSCALL` record
carries `key`, `comm`, `exe`, `auid`; the `PATH` record carries `name`. Three rules
(`ssh_authorized_keys_write`, `ssh_private_key_read`, `history_clearing`) match fields
from both in one selection, so they assume an ingestion that **coalesces** the records
of one event into one document — auditbeat/Elastic and the Splunk auditd TA do. A raw
per-line parser never sees `key` and `name` on the same line and leaves those three
rules inert while the key-only rules beside them keep working. The fixtures are written
in the coalesced shape; confirm your pipeline produces it before trusting these three.

**Argument positions.** Where a documented audit rule tests a syscall argument
(`-F a1&04000`, `-F a2&01000`), the position is per syscall, not per family: `fchmodat`
carries the mode in `a2` where `chmod`/`fchmod` carry it in `a1`, and `openat` carries
its flags in `a2` where `open` carries them in `a1`. `suid_bit_set` and
`history_clearing` ship one line per position for that reason; a single line across the
family tests a pointer against a bitmask on the calls it gets wrong.
...
| `azure_keyvault_bulk_secret_read` | Key Vault `AuditEvent` SecretGet, distinct secrets per oid (value_count correlation)    | T1555.006 | Azure KV · azure-keyvault-secret-dump    |
...
| `cloud/entra_directory_role_grant`                               | `filter_iga` → the **object id** of your identity-governance / lifecycle automation principal         | PIM and IGA workflows are the noisiest AuditLogs operations there are, so unfilled this reports your joiner-mover-leaver traffic. Don't allowlist PIM itself, and use the object id — a display name breaks on a rename      |
